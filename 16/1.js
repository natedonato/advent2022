  ...
  if(node === 'AA' || graph[node].rate > 0){
    ...
    for(const node2 of Object.keys(graph)){
      if(node2 !== node && graph[node2].rate > 0){
        paths[node].push({valve: node2, dist: calculateShortestPath
        (node, node2)})
      }
    }
  } 
...
while(queue.length > 0){
  ...
  let children = paths[valve].filter(el => !openValves.includes(el.valve) && remainingTime >= el.dist)
  children.forEach(child => {
    let newlyReleased = (child.dist) * releasingRate;
    queue.push([child.valve, [...openValves, child.valve], released + newlyReleased, minute + child.dist, releasingRate]);
  })
